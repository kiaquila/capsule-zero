...
> **Q8 freeze (2026-07-24):** shared merchant-image import/search is retained product intent, not an
> active entrance-gate surface. Marketplace import, shared catalog/search, moderation, embeddings,
> merchant storage prefixes, and generated-client types remain absent until a compliance-scheme
> spec and external legal review are both complete. Own-imagery preset catalog work is separate.
...
| Production server               | ~~DO droplet ≥ 4 GB / 2 vCPU / 80 GB~~ Hetzner CX23 (2 vCPU / 4 GB / 40 GB) — superseded 2026-07-02, spec 033 | Server provisioned; DNS A record for `capsulezero.app` pointed directly at the server IP                                                                                                                                 |
| DNS + Cloudflare proxy          | Active production edge (spec 047, 2026-07-22)                                        | Spaceship registrar → Cloudflare nameservers; apex + `www` proxied; Full (strict) TLS + DNSSEC; Cloudflare-only origin web ingress; scoped rate limit and default WAF/DDoS controls. Bot Fight Mode stays disabled because it challenged the health monitor |
...
| API contract                    | `docs_capsule_zero/adr/openapi.yaml` plus updated `docs_capsule_zero/adr/api-spec.md` | P0: every active route-method has auth, request, response, and error schemas; Q8-blocked merchant import/search/moderation surface is absent and guarded against early reintroduction                                 |
...
| Authorization tests             | `api/tests/authz/`                                                                    | Cross-user mutation rejected; any public read is limited to Capsule Zero-owned preset data; Q8-blocked merchant entries have no route or table                                                                                                |
| Storage                         | Hetzner Object Storage buckets + CORS                                                 | Buckets exist; production CORS allows exactly `https://capsulezero.app`; signed PUT/HEAD/GET/delete round-trip verified against the chosen region; no wildcard origin; no merchant-import prefix before both Q8 gates                                                    |
...
| Backups                         | Root-owned systemd timer outside compose (spec 047)                                   | Daily client-side encrypted `pg_dump` + configuration/security manifest uploads to the Hetzner backup bucket; Object Lock posture, off-server private key, fixed uploader headers, and a full restore drill verified |
...
| OpenAPI completion              | Expand `openapi.yaml` only for unblocked route-methods. Marketplace imports/shared search return after both Q8 gates, in the same change as their compliance-approved contract.           |
...
| Vector dimension                | Lock the pgvector embedding dimension before an approved search slice writes embeddings; shared merchant search additionally requires both Q8 gates.                                                                                |
...
| Rate limits and cost ceilings   | Define photo-upload limits now; marketplace-import/shared-search limits belong to the post-gate Q8 contract.                                                                                              |
...
Phase 5 feature sprints may start when the Sprint 0 gates above are complete. If a feature requires Google/Apple OAuth in Kratos, real Lava.top payment, or the self-hosted image model, run the corresponding integration gate before that feature enters QA, staging, or launch. Marketplace import or shared merchant-catalog search additionally cannot enter implementation until the Q8 compliance-scheme spec and external legal review are both complete.
