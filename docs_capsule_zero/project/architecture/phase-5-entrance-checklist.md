...
The Phase 4 production-stack architecture is accepted. Phase 5 starts with a production-shape docker-compose runtime and real services from Day 1 (see ADR-006). `/app` remains the canonical provider-abstracted web frontend; the retired Supabase provider is removed domain by domain as Go API contexts land. Real provider registration (Google/Apple OAuth, Lava.top live, self-hosted image model) is a per-feature integration gate, not a precondition for the runtime.
...
| docker-compose runtime          | `docker-compose.yml` plus `docker-compose.dev.yml`                                    | Every service from the production runtime table is declared as a separate `services:` block; `docker compose up -d` brings the full stack up and every service reports healthy          |
...
| Generated clients               | `app/src/lib/api/generated/` and `mobile/lib/api/generated/`                          | TypeScript clients for web and React Native regenerated from the OpenAPI contract                                                                                                       |
...
| Schema migrations               | `api/migrations/`                                                                     | Tables, indexes, FKs, enum/check constraints, seed methodology data shipped as versioned SQL files; migrations apply cleanly at API boot                                                |
...
| React Native scaffold           | `/mobile`                                                                             | Expo project boots, routing covers `(auth)` and `(app)` groups, env config covers API base URL and deep-link scheme; build runs locally for at least one platform                       |
...
| Observability expansion         | Bring Sentry and Prometheus into Stage 2; until then Grafana + syslog + traces is the observability surface.                                                         |
