...
| DNS + Cloudflare proxy          | **Deferred to Stage 2** (founder decision 2026-07-02, spec 033) — not a Phase 5 gate  | v0.1 runs direct DNS with Let's Encrypt TLS on host nginx. On Stage-2 activation: Spaceship nameservers → Cloudflare, proxy (orange cloud) on the apex, SSL/TLS `Full (strict)`, Bot Fight Mode                          |
...
| Backups                         | Cron job in the runtime                                                               | Nightly client-side encrypted `pg_dump` uploads to the Hetzner backup bucket with at least 14 day retention / Object Lock posture verified                                                                               |
