...
| DI-003 | Auth | Supabase Auth with email/password, Google OAuth, and Apple Sign-In | 5/5 approve | Matches MVP auth requirements across web and Flutter while keeping session identity tied directly to Postgres RLS. |
...
| DI-010 | Web i18n | next-intl | 5/5 approve | Best fit for Next.js App Router, Server Components, and the EN/ES-AR/RU requirement from Day 1. |
...
- Complete `docs_capsule_zero/project/architecture/phase-5-entrance-checklist.md` Sprint 0 before feature implementation.
- Add linting and local commit hooks before Phase 5 product-code PRs.
- Create Supabase project, storage buckets, OAuth providers, and local seed data.
...
- Confirm Photoroom response quality and latency against real wardrobe photos. If it misses the 5 second processing gate or quality bar, switch the adapter to remove.bg for v0.1.
