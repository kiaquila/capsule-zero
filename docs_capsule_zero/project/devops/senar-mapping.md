...
| Evidence-based verification | `## Verification` table in `plan.md` (AC → Evidence), plus `npm run preflight`, `baseline-checks`, `guard`, and `test` |
...
- The standard merge-ready conditions also hold: green required checks (`baseline-checks`, `guard`, `test`), no blocking review findings on the head SHA, no merge conflicts.
