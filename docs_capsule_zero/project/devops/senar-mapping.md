...
- **Done-Gate judgment** (negative-scenario waiver, accepted known issues, process memory quality) is enforced by the review agent and the human merge owner.

No change to `pr-guard.yml`, `ai-review.yml`, `ci.yml`, `ai-command-policy.yml`, `osv-scan`, or branch-protection approvals. The only new automated gate is `test`.
...
- **Judgment**: waiving a negative scenario, accepting a known issue, and grading the quality of process memory remain human/review-agent calls. They are confirmed via the SENAR Done Gate checklist in the PR template, not via regex parsing in CI.
