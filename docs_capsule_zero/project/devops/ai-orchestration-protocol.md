...
- If Codex has no concrete findings, the connector may emit a top-level `Codex Review:` comment instead of a formal PR review; the gate treats that native no-findings reply as a pass when its `Reviewed commit` anchor matches the current head SHA or, for anchorless replies, when it lands inside the active cycle.
- Any untagged Codex inline finding fails the gate closed, and reruns may reuse an already-published valid Codex review or head-SHA-anchored no-findings summary for the same head SHA.
