...
| 9 | `osv-scan` green on PR head | The original `GO-2026-5856` follow-up pinned `1.25.12`. PR #101 first tested `1.25.13`, which fixed five later advisories, but `GO-2026-5942` has no fixed 1.25.x release. `api/go.mod`, `api/Dockerfile`, `docker-compose.yml`, `deploy/compose.env.example`, and `.github/workflows/test.yml` therefore pin the first fixed stable toolchain, `1.26.6`. CI `osv-scan` is the canonical PR-head evidence |
