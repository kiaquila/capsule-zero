...
| 9 | `osv-scan` green on PR head | The original `GO-2026-5856` follow-up pinned `1.25.12`; superseded 2026-08-13 after the PR #101 gate found later stdlib advisories fixed in `1.25.13`. `api/go.mod`, `api/Dockerfile`, `docker-compose.yml`, `deploy/compose.env.example`, and `.github/workflows/test.yml` now pin `1.25.13`. CI `osv-scan` is the canonical PR-head evidence |
