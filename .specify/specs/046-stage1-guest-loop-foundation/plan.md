...
| AC-012 | `npm ls brace-expansion postcss --all --package-lock-only --prefix app` → `postcss@8.5.18`, `brace-expansion@5.0.8` и только legacy `1.1.16`; e2e tree → `5.0.8` + legacy `1.1.16`. Оба `osv-scanner.toml` совпадают по `name/version/ecosystem`, содержат `effectiveUntil = 2026-08-08` и не используют ID-ignore. `npm audit --audit-level=high` ожидаемо остаётся красным только на legacy 1.1.16, потому что npm audit не читает OSV config; обязательный `docker run ... ghcr.io/google/osv-scanner-action:v2.3.5 --recursive --experimental-exclude worktrees .` загрузил оба точечных фильтра → **No issues found / Exit code 0** | ✅ |
