...
| AC-013 | RED contract probe до sweep: `/api/imports/marketplace`, `/api/catalog/search`, moderation paths, `sourceType=marketplace`, `catalogItemId`, merchant schemas/jobs присутствовали в OpenAPI и generated client. Fail-closed mutation после sweep: временный возврат `SourceType=marketplace` заставил `node scripts/check-api-contract.mjs` завершиться с `SourceType marketplace is blocked by PRODUCT-PLAN Q8` / exit 1. После восстановления `npm run generate:api` → **44 operations**; `npm run check:api-contract` подтверждает отсутствие blocked surface и синхронный generated client. Canonical Q8 manual list + inventory прогнаны; active decision docs отличают retained design от executable contract, historical spec/frozen provider/migrations остаются superseded legacy | ✅ |
