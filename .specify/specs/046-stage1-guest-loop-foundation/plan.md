...
На 2026-07-24 база advisories добавила ещё два сигнала: исправимые `postcss<=8.5.17` и
`brace-expansion<=5.0.7`, а legacy `brace-expansion@1.1.16` не имеет совместимого patched release.
T020 обновляет исправимые версии и добавляет точные version-scoped OSV package overrides рядом с
двумя lockfiles до 2026-08-08; broad advisory ignore и ослабление workflow не используются.
T021 закрывает найденный native review doc/contract drift после Q8: retained product stories
остаются, но заблокированный merchant-image surface удаляется из OpenAPI/codegen и защищается
negative contract guard; manual canonical list + inventory sweep актуализируют active docs.
...
| AC-011 | `npm ls sharp fast-uri --all` → только `sharp@0.35.3 overridden` и `fast-uri@3.1.4 overridden`; исторический `npm audit --audit-level=high` до нового brace advisory → **0 vulnerabilities**; текущий audit больше не сообщает `sharp`/`fast-uri`, но отдельно показывает только legacy finding из AC-012. `ghcr.io/google/osv-scanner-action:v2.3.5` с CI-аргументом `--recursive .` → **No issues found / Exit code 0**; `npm run ci:check` и production Docker build → pass | ✅ |
| AC-012 | `npm ls brace-expansion postcss --all --package-lock-only --prefix app` → `postcss@8.5.18`, `brace-expansion@5.0.8` и только legacy `1.1.16`; e2e tree → `5.0.8` + legacy `1.1.16`. Оба `osv-scanner.toml` совпадают по `name/version/ecosystem`, содержат внешний `PackageOverrides.effectiveUntil = 2026-08-08` по schema тега v2.3.5 и не используют ID-ignore. `npm audit --audit-level=high` ожидаемо остаётся красным только на legacy 1.1.16, потому что npm audit не читает OSV config. Fail-closed mutation: временная замена expiry на `2026-07-24` загрузила оба filter и вернула **2 High / Exit code 1**; после восстановления даты обязательный `docker run ... ghcr.io/google/osv-scanner-action:v2.3.5 --recursive --experimental-exclude worktrees .` → **No issues found / Exit code 0** | ✅ |
| AC-013 | RED contract probes: `/api/imports/marketplace`, `/api/catalog/search`, moderation paths, `sourceType=marketplace`, `catalogItemId`, merchant schemas/jobs присутствовали в OpenAPI/generated client; временный возврат `SourceType=marketplace` дал `SourceType marketplace is blocked by PRODUCT-PLAN Q8` / exit 1; новый ProviderRegistry guard отдельно упал на унаследованном mock-порте с `ProviderRegistry.marketplaceImports is blocked by PRODUCT-PLAN Q8`. UI RED commit `f49da1e`: Playwright получил enabled `Paste Links`, ожидая disabled. Review RED `9829676`: contract guard поймал executable Supabase `buildCatalogSearchPort`, Chromium/WebKit не нашли объяснение blocked control. Legal RED `cb84feb`: Terms/Privacy не содержали gate и продолжали обещать активный URL import/adapter processing. GREEN: marketplace port и рабочая mock-реализация/fixture/handler удалены; frozen Supabase marketplace + shared-catalog paths недостижимы и fail-closed; Links tab disabled, direct query не активирует форму, EN/RU copy объясняет gate и upload/preset alternatives; live legal pages на EN/RU routes условно описывают будущую фичу, текущий no-collection/no-disclosure и предварительное обновление policies; `npm run generate:api` → **44 operations**, `npm run check:api-contract` подтверждает отсутствие blocked surface и синхронный client; targeted Chromium + WebKit Q8/legal specs, app typecheck/lint/build → pass. Canonical Q8 manual list + inventory прогнаны; active decision docs отличают retained design от executable contract, historical spec/frozen provider/migrations остаются superseded legacy | ✅ |
| Review | Native Codex шесть P2 и пять P1 (один P1 опровергнут provider probe `46/40/48 ≥ 8`) + code-reviewer + architect: доковые findings закрыты в T008, runtime display drift — в T009, неполный 31/48 runtime category catalog — в T010, canonical gender/provider projection — в T011, builder-level regression efficacy — в T012, persisted numerator drift — в T013, persisted-ID dedup test efficacy — в T014, Node-20 CI portability — в T015, deterministic accessory selection — в T016, schema-conformant Capsule example — в T017, exact representative rendering — в T018; Q8 contract/UI/legal sweep и финальные Supabase/explanation gaps — в T021; повторный `@codex review` запускается после push каждого fix-цикла | ✅ |
