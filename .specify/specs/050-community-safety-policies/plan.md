...
| 8   | Repository and app gates pass                                                          | Final `CI=1 npm run preflight` passed after the protected-layout refactor: feature memory, repo baseline, API contract, app/e2e lint, CSS lint, both typechecks, production build (41 routes), and browser suite (98 passed, 8 environment-dependent skipped); warning-only lint baselines remained non-blocking. The Node 20 compatibility follow-up also passed app/e2e typechecks, production build, test collection, and the focused policy matrix |
