...
| 8   | Repository and app gates pass                                                          | Final `CI=1 npm run preflight` passed after RU future-Terms localization: feature memory, repo baseline, API contract, app/e2e lint, CSS lint, both typechecks, production build (41 routes), and browser suite (104 passed, 8 environment-dependent skipped); warning-only lint baselines remained non-blocking. The Node 20 compatibility follow-up also passed app/e2e typechecks, production build, test collection, and the focused policy matrix |
...
| 17  | Future Terms are substantively available in Russian                                    | Red-first Chromium evidence received the English H1 on the RU preview. The complete Russian future contract now exposes all 22 sections and the required ownership, license, intermediary, incorporated-policy, user-content-disclaimer, liability-cap, and indemnity language; the focused preview plus effective-instant resolver scenario passed in Chromium and mobile WebKit, and route metadata follows the locale                              |
...
feature pages. Resume-time verification reuses a no-store `HEAD` to the current
