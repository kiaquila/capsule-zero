...
| 8   | Repository and app gates pass                                                          | Final `CI=1 npm run preflight` passed after the session-deduplication fix: feature memory, repo baseline, API contract, app/e2e lint, CSS lint, both typechecks, production build (41 routes), and browser suite (108 passed, 8 environment-dependent skipped); warning-only lint baselines remained non-blocking. The later infrastructure-only Go follow-up passed local OSV Scanner v2.3.5, API vet/tests, and Compose config; exact Node 20 collection, the focused reconnect 10/10 matrix, and the focused session contract 2/2 matrix also passed across Chromium and mobile WebKit |
...
The existing app and e2e `osv-scanner.toml` files were checked before adding the API-
local configuration; neither can own a Go-module-only package mismatch without applying
the exception to unrelated dependency manifests.
