...
Authenticated Stage 1 users can open a localized mock-first Favorites screen, review favorite wardrobe and catalog-derived items in the approved two-section layout, filter them by category, remove favorites locally, and edit favorite item details using the same local detail-card behavior already accepted for wardrobe screens.
...
- Render the approved Favorites shell: desktop sidebar, topbar, language switcher, My Items / From Catalogs tabs, category filters, favorite item grid, shared My Items-style favorite controls, catalog badges, mobile bottom nav, and more sheet.
- Support local tab switching, category filtering, item detail editing, local photo preview upload, local Save, local Remove Favorite, local Move to Sale, and local Move to Repair.
...
- Deleting wardrobe items from Favorites; this slice removes the favorite marker locally rather than deleting the object.
...
### User Story 2 - Filtering and Remove Favorite (Priority: P2)

As a user, I want to filter Favorites and remove an item from Favorites without leaving the screen.

**Why this priority**: Favorites is a quick-return list and should stay fast and reversible in Stage 1.

**Independent Test**: Use category filters on `/en/favorites`, remove a favorite with the active heart, and verify the grid/counts update without page reload or provider writes.
...
2. **Given** a favorite card is visible, **When** the favorite control is clicked, **Then** the item leaves the Favorites grid locally and the favorite count decreases.
3. **Given** a category filter has no visible favorites, **When** the grid rerenders, **Then** a localized filtered empty state appears.
4. **Given** the page is reloaded, **When** the mock fixtures are read again, **Then** local favorite removals reset because Stage 1 does not persist them.
...
**Independent Test**: Open a favorite detail, edit and save fields, validate name errors, use local photo preview, remove the favorite, and move an item to Sale/Repair in separate page sessions; verify all changes are client-local.
...
6. **Given** an item is moved to sale or repair, **When** the action is clicked, **Then** its local status and navigation counts update and the action is explained with a localized notice.
...
- A user removes the last favorite in a tab.
...
2. **Given** Stage 1 remains mock-first, **When** edit, save, remove favorite, photo preview upload, sale, or repair actions are used, **Then** they update client-local preview state only and do not call real Supabase, storage, marketplace, semantic search, Photoroom, or Lava.top providers.
...
- **FR-008**: Category filters MUST update the active tab grid client-side without page reload.
...
- **FR-010**: Item detail MUST provide Save, Remove Favorite, Move to Sale, and Move to Repair controls with localized validation and notices.
...
- **LocalFavoritesPreview**: Client-side state for tab switching, category filtering, editable detail panel, local photo object URL preview, local status transitions, favorite removal, and notices.
...
- **SC-004**: Category filters and section tabs update the grid without page reload.
- **SC-005**: Active heart removal updates the preview and count without provider writes.
- **SC-006**: Detail edit/save/photo/remove/sale/repair interactions update the preview without provider writes.
