...
| 5 | `/api/health` contract stays in sync: OpenAPI + markdown contract + generated client expose `commit`/`builtAt` | `docs_capsule_zero/adr/openapi.yaml` — `getHealth` → `HealthResponse` (200 + 503); `docs_capsule_zero/adr/api-spec.md` route row updated (build identity + 503); `npm run generate:api` regenerated `app/src/lib/api/generated/openapi.ts`; `npm run check:api-contract` PASS (52 operations) |
