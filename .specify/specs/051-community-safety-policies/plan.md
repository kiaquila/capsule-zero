...
| Accurate Privacy safeguards | Commit `01c5b0f`; current EN/RU Privacy pages failed red while they claimed row-level security. The published safeguards now match Go API ownership checks, signed private-media URLs, granular permissions, and encrypted database backups. |
...
The RU Privacy module follows the established `terms-current-ru.ts` locale-document
pattern; the route selects between EN and RU while continuing to reuse `LegalPage`.
