...
| SC-004 / FR-005-FR-008, FR-015 | Browser tab smoke switched Items, Outfits, What's Missing, and Shopping List without page reload; each tab rendered its expected result content. Follow-up Browser check verified the Outfits view toggle uses prototype SVG icons, line view renders all outfit layers in a flex row, and square view renders a 2x2 collage card. Codex review follow-up verified direct `/en/capsule-result?tab=outfits` selection and tab-button navigation to `/en/capsule-result?tab=shopping` keep the active tab and rendered panel synchronized. |
