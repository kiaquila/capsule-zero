...
| SC-004 / FR-005-FR-008, FR-015-FR-016 | Browser tab smoke switched Items, Outfits, What's Missing, and Shopping List without page reload; each tab rendered its expected result content. Follow-up Browser check verified the Outfits view toggle uses prototype SVG icons, line view renders all outfit layers in a flex row, and square view renders a 2x2 collage card. Codex review follow-up verified direct `/en/capsule-result?tab=outfits` selection and tab-button navigation to `/en/capsule-result?tab=shopping` keep the active tab and rendered panel synchronized. Shopping rows now land on Guided Journey step 3 search with category-prefilled query state and a valid mixed setup containing 8 selected categories. |
| SC-005 / FR-009-FR-010, FR-014 | Browser interaction smoke toggled favorite `aria-pressed`, added compatible `Camel wool blazer`, removed it through confirmation, recalculated `3 items / 27 outfits` then returned to `2 items / 18 outfits`, and showed `Blush silk scarf` as palette-blocked with visible reason. Codex review follow-up verified Add picker candidates include `Camel wool blazer` and exclude `Black leather loafers` while retaining catalog candidates. |
| SC-006 / FR-011, FR-017 | Browser DOM on `/ru/capsule-result` rendered Russian labels with `<html lang="ru">`; EN/RU language controls were present and no ES-AR/Spanish control text was present. Known Stage 1 provider gap reasons are mapped to RU copy before rendering. |
