...
**Constraints**: must not modify `/app` beyond `data-testid` additions; must not touch other workflows; must not require real third-party credentials
...
- **Out of scope**: branch-protection admin step (follow-up), Go/RN tests (later specs), product UI changes, retrofitting grandfathered specs.
...
| FR-010               | Devops workflow and branch-protection docs under `docs_capsule_zero/project/devops/` list `test` as a required check where merge readiness is defined.                                                                 |
...
**Structure Decision**: top-level `tests/` workspace with one folder per testing surface (e2e / unit / mobile). `/app` is the canonical target of e2e; the POM classes isolate selectors from future component rewrites.
...
- **Risk**: Tests bound to early `/app` components may break as provider-backed screens are rewritten. **Mitigation**: POM isolates selectors; keep stable `data-testid` contracts when components move or split. Documented in `tests/README.md`.
