...
- [x] T057 Live verification: local Docker stack (`docker compose --env-file deploy/compose.dev.env -f docker-compose.yml -f docker-compose.dev.yml`) built and served the web container; `curl` of `/icon.svg` and `/favicon.ico` byte-identical to the repo files, landing `/en` = 200; rendered `icon.svg` in a Chromium engine at forced `prefers-color-scheme: light` (dark letters) and `dark` (light letters); 16/32px tab-strip context montage over light and dark rest/hover/active surfaces reviewed. Founder approved the local preview and authorized taking it to prod.
...
- **Achromatic theme-adaptive over the gold plate (founder decision, 2026-07-18).** After living with the plated F1 mark in production the founder judged it unsatisfactory and asked to keep the V5 letterforms but recolor them the way the *first* favicon (PR #59) worked — dark ink on light chrome, light ink on dark chrome. This supersedes the F1 plate and the all-gold fill for the favicon. Rationale honored faithfully: the letterforms, typography, and size come from the previous variant (V5, byte-identical paths); only the color mechanism is swapped back to PR #59's achromatic `#1C1C1C`→`#EDEDED` under `prefers-color-scheme`.
...
- **ICO bakes the light-theme dark ink `#1C1C1C`.** Same posture as PR #59 and the gold iteration: ICO cannot theme-adapt, light chrome is the majority default, and dark-chrome clients honoring SVG favicons get `#EDEDED` from `icon.svg`.

### Known Issues

- **Two-letter density at 16px.** "CZ" is wider and shorter than the single "C.", so at the 16px browser-tab size the letters are dense — legible on both themes in the SC-009 montage, but small. Inherent to the V5 letterforms the founder chose to keep. Removing the plate returns the letters to full size (no 0.82 down-scale), which is *better* at 16px than F1; larger sizes (32/48/SVG) are crisp.
- **Gold-on-white contrast.** *(Moot from 2026-07-18.)* The V5-era low gold-on-light contrast — fixed by the F1 plate on 2026-07-17 — no longer applies: the mark is achromatic. Dark `#1C1C1C` ink has full contrast on a light strip, and the `prefers-color-scheme: dark` step gives light `#EDEDED` ink full contrast on a dark strip. No plate is needed and none is shipped.
