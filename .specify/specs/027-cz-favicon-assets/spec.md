...
- The SVG "CZ" monogram sits on a dark rounded-square plate: plate `#0A0A0A`, hairline stroke `rgba(255,255,255,.22)` (so the plate stays visible on dark tab strips), letters flat gold-500 `#EFBF04` — the landing logo's gold on the landing hero's near-black. The plate makes the mark theme-independent, so the former `prefers-color-scheme` gold step is removed.
...
2. **Given** the icon assets are inspected, **When** the SVG is checked, **Then** it carries the dark plate `#0A0A0A` with hairline `rgba(255,255,255,.22)` and the "CZ" paths carry flat gold-500 `#EFBF04` fills — no `prefers-color-scheme` rule, no retired yellow, no off-token fill.
...
- **FR-006**: `app/src/app/icon.svg` MUST use only the founder-approved F1 palette — plate `#0A0A0A`, hairline `rgba(255,255,255,.22)`, letters flat gold-500 `#EFBF04` — never an off-token gold, never yellow `#FFD600`, and never the retired achromatic C fills (`#1C1C1C`/`#EDEDED`). The former `prefers-color-scheme` gold step (`#FFDD00`) MUST be absent: the plate makes the mark theme-independent, and a stale media query would signal drift from the approved F1.
...
- **SC-008**: The shipped SVG reproduces the founder-approved F1 gallery composition exactly — a same-pipeline 512px raster of `app/src/app/icon.svg` vs the F1 gallery source differs by **0 pixels** (`magick compare -metric AE` = 0).
