...
- Asset validation evidence for SVG validity, the achromatic theme-adaptive palette (dark/light ink, the media query), byte-identity of the letters vs V5, fallback ICO sizes, and changed paths.
...
As a visitor, I want the browser tab and saved-page icon to use the Capsule Zero "CZ" mark, legible on whatever browser theme I run, so the app has a clear brand signal outside the page content.
...
3. **Given** a user runs a dark browser theme, **When** the favicon renders over the dark tab strip, **Then** the SVG must switch to the light `#EDEDED` ink via the `prefers-color-scheme: dark` rule so the near-black `#1C1C1C` mark cannot disappear on the dark strip.
...
- **FR-006**: `app/src/app/icon.svg` MUST use only the achromatic theme-adaptive palette — base fill `#1C1C1C`, dark-scheme fill `#EDEDED` under `@media (prefers-color-scheme: dark)` — and MUST NOT contain the gold letters (`#EFBF04`/`#FFDD00`), the dark plate (`#0A0A0A` or any `<rect>`), or the retired yellow (`#FFD600`). The `prefers-color-scheme` rule MUST be present: this iteration restores the first favicon's theme adaptation.
- **FR-007**: `app/src/app/icon.png` MUST stay removed so the modern metadata icon source is unambiguous.

### Key Entities

- **Favicon ICO**: Browser favicon asset served through the Next.js App Router `favicon.ico` convention (light-theme dark ink baked).
...
- **SC-009**: The mark is legible on both themes — a light/dark × rest/hover/active tab-strip context montage (16px at 4×, plus 32px) shows dark `#1C1C1C` letters clearly on the light strip and light `#EDEDED` letters clearly on the dark strip, verified live in a browser engine at forced `prefers-color-scheme: light` and `dark`.
