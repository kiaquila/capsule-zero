...
| 9q | 2 | Auth write throttling also covers requests forwarded by the Next.js server actions over the private network (which never traverse the edge `limit_req`): the Go API rate-limits `/api/auth/{registration,login,recovery}` per originating client (10/min, burst 10 — mirroring the edge), and the web `api` provider forwards the real client IP so the limit buckets per user, not per web container | ✅ `docker run --rm -v "$PWD/api:/src" -w /src golang:1.25.11-bookworm go test ./...` passes `internal/ratelimit` (burst-then-429, time-based refill, per-key isolation, idle-bucket eviction, 429 envelope + `Retry-After`, distinct forwarded clients, RemoteAddr fallback); ✅ `go vet ./...` clean; ✅ `npm run typecheck` + `npm run lint` cover `apiFetch` forwarding `x-forwarded-for`/`x-real-ip` to the Go API |
