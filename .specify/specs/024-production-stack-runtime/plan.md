...
- **Retired Supabase provider still exists in some `/app` domains.** Until each bounded context moves to the Go API provider, some code paths may still import Supabase modules. Mitigation: keep the provider frozen, default runtime to `api`, and remove Supabase domain by domain in Phase 6/follow-up slices.
