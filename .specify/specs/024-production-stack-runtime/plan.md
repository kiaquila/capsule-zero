...
| 4   | 1     | On the droplet: host nginx owns the edge — `systemctl is-active nginx` returns `active` and `systemctl is-active caddy` returns `inactive`; `docker compose ps` lists `web` as `(healthy)` within 60 s of `docker compose up -d` (the rollback compose edge is exercised separately via `docker compose --profile docker-edge up -d nginx`) | shell output committed to PR description after first droplet rollout                                     |
