...
| 9q | 2 | Auth write throttling also covers requests forwarded by the Next.js server actions over the private network (which never traverse the edge `limit_req`), and keys on a **spoof-resistant** client address: the Go API rate-limits `/api/auth/{registration,login,recovery}` per originating client (10/min, burst 10 — mirroring the edge) on the trusted `X-Capsule-Client-IP` header only; the web boundary derives that header from `CF-Connecting-IP`/`X-Real-IP` (never raw client `X-Forwarded-For`), and nginx overwrites any client-supplied value on `/api/*`, so a caller cannot rotate the header to mint a fresh bucket | ✅ `docker run --rm -v "$PWD/api:/src" -w /src golang:1.25.11-bookworm go test ./...` passes `internal/ratelimit` (burst-then-429, time-based refill, per-key isolation, idle-bucket eviction, 429 envelope + `Retry-After`, distinct trusted clients, **raw `X-Forwarded-For` ignored**, trusted-header-preferred, RemoteAddr fallback); ✅ `go vet ./...` clean; ✅ `nginx -t` confirms both configs set `X-Capsule-Client-IP $remote_addr` on `/api/*`; ✅ `npm run typecheck` + `npm run lint` cover `apiFetch` deriving the IP from `cf-connecting-ip`/`x-real-ip` |
