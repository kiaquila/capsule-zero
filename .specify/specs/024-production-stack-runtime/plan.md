...
| 9j | 2 | Embedded SQL migrations execute multi-statement files without pgx prepared-statement rejection, and the implemented auth/profile API is published in the canonical OpenAPI + generated clients | ✅ `docker run --rm -v "$PWD/api:/src" -w /src golang:1.25.11-bookworm go test ./...` covers the simple-protocol migration executor; ✅ `npm run check:api-contract` verifies `/api/auth/*`, profile schemas, and regenerated web/mobile clients |
