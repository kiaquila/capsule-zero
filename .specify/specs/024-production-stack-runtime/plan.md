...
| 9i | 2 | Kratos compose startup uses production-safe env/health semantics: indexed secret env vars override committed dev defaults, and the healthcheck runs a binary present in `oryd/kratos:v1.1.0` | ✅ `docker run --rm oryd/kratos:v1.1.0 remote status --help`; ✅ `docker compose --env-file deploy/compose.env.example config` shows `SECRETS_COOKIE_0`, `SECRETS_CIPHER_0`, and exec-form `kratos remote status` healthcheck |
