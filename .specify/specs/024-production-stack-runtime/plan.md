...
| 9i | 2 | Kratos compose startup uses production-safe env/health semantics: base compose requires real indexed secrets, production SMTP, and public URLs; production Kratos does not run with `--dev`; flow UI URLs are public app routes; the healthcheck runs a binary present in `oryd/kratos:v1.1.0` | ✅ `docker run --rm oryd/kratos:v1.1.0 remote status --help`; ✅ `docker run --rm oryd/kratos:v1.1.0 serve --help` documents `--dev` as disabling critical security features; ✅ `docker compose --env-file deploy/compose.env.example config` shows `SECRETS_COOKIE_0`, `SECRETS_CIPHER_0`, Resend-shaped `COURIER_SMTP_CONNECTION_URI`, `SERVE_PUBLIC_BASE_URL=https://capsulezero.app/`, public `SELFSERVICE_*` URLs, `command: serve --config /etc/config/kratos/kratos.yml --watch-courier`, and exec-form `kratos remote status` healthcheck; ✅ negative compose config without `SECRETS_COOKIE_0` / `SECRETS_CIPHER_0` / `KRATOS_SMTP_CONNECTION_URI` fails before rendering |
...
| 9l | 2 | Session verification, protected-route middleware, and web sign-out do not hide identity-provider outages: invalid/expired sessions still resolve empty/401, Kratos whoami outages return 502, and the web `api` provider throws before clearing the local session when logout fails | ✅ `docker run --rm -v "$PWD/api:/src" -w /src golang:1.25.11-bookworm go test ./...` covers whoami invalid-token vs upstream-error classification, `RequireSession` invalid-token vs upstream-error classification, and handler 502 behavior; ✅ `npm run lint`; ✅ `npm run typecheck`; ✅ `npm run build` cover the web provider status checks |
...
| 9n | 2 | The web container receives only its explicit runtime allowlist and does not inherit backend secrets from the shared compose env file | ✅ `docker compose --env-file deploy/compose.env.example config --format json` inspected `services.web.environment` and found no `POSTGRES_*`, `KRATOS_*`, `SECRETS_*`, `*_DATABASE_URL`, or SMTP secrets |
...
| 10 | 2 | nginx routes public `/api/*` traffic to the Go API and Kratos public `/self-service/*` + `/sessions/*` traffic to Kratos in both supported edge paths: Docker nginx (`api:8080`, `kratos:4433`) and active host/systemd nginx (`127.0.0.1:8080`, `127.0.0.1:4433`) | ✅ `docker compose --env-file deploy/compose.env.example config`; ✅ `nginx:1.27 nginx -t` against `infra/nginx-host/00-capsule-zero.conf` + `infra/nginx-host/capsulezero.app.conf` with a temporary test cert |
