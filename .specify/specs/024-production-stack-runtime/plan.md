...
| 9h | 2 | Unexpected Kratos recovery/courier failures surface as a temporary 502 instead of a false success, while validation-style recovery responses remain account-enumeration safe | ✅ `docker run --rm -v "$PWD/api:/src" -w /src golang:1.25.11-bookworm go test ./...` covers recovery status mapping and handler 502 behavior |
