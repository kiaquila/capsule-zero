...
| 9i | 2 | Kratos compose startup uses production-safe env/health semantics: base compose requires real indexed secrets and public URLs, flow UI URLs are public app routes, and the healthcheck runs a binary present in `oryd/kratos:v1.1.0` | ✅ `docker run --rm oryd/kratos:v1.1.0 remote status --help`; ✅ `docker compose --env-file deploy/compose.env.example config` shows `SECRETS_COOKIE_0`, `SECRETS_CIPHER_0`, `SERVE_PUBLIC_BASE_URL=https://capsulezero.app/`, public `SELFSERVICE_*` URLs, and exec-form `kratos remote status` healthcheck; ✅ negative compose config without `SECRETS_COOKIE_0` / `SECRETS_CIPHER_0` fails before rendering |
