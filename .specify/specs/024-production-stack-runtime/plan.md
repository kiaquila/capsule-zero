...
| 9c | 2 | **Registration + login work end-to-end on the existing `/app` UI** with `CAPSULE_PROVIDER_MODE=api`: sign-up creates a Kratos identity and a mapped `profiles` row; login establishes a session; `/dashboard` is reachable | ✅ Playwright e2e `specs/auth/registration.spec.ts` (2 passed): sign-up lands on `/dashboard`; sign-in with the same credentials lands on `/dashboard`. Go-API smoke: `profiles` row present (`profile.userId == user.id`); PATCH persists `displayName`/`city` |
| 9d | 2 | Smoke sign-up delivers a verification email (MailHog in dev; real Resend gated to Phase 4) and the `api` provider surfaces a safe inline error on a failed flow | ✅ MailHog received "Please verify your email address" for the new identity; wrong-password login → Go API `401 invalid_credentials`, surfaced inline |
