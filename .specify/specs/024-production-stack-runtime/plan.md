...
8. Add `kratos` to compose with Resend SMTP courier in prod and MailHog in `docker-compose.dev.yml` (reintroduced this phase). `infra/kratos/` identity schema (`traits.email`, `traits.name.first`, `traits.locale`) + self-service flows for sign-up, sign-in, and settings. Recovery and email-verification flows are **disabled** in this slice (no flow-aware completion UI yet — see Process Memory); they return with the dedicated recovery/verification slice.
...
| 9d | 2 | Sign-up completes without a dead-end email step and the `api` provider surfaces a safe inline error on a failed flow. Email verification is **deferred** with recovery (no flow-aware completion UI in this slice), so sign-up auto-logs-in via the Kratos `session` hook and sends no verification email | ✅ `infra/kratos/kratos.yml` sets `verification.enabled: false`; sign-up → session → `/dashboard` (Playwright `specs/auth/registration.spec.ts`); wrong-password login → Go API `401 UNAUTHENTICATED`, surfaced inline; duplicate email → generic `400 VALIDATION_ERROR` with no account-existence disclosure (`TestRegistrationDoesNotLeakAccountExistence`) |
...
| 9h | 2 | Password recovery is **deferred** to a dedicated slice rather than shipped half-working: the auth UI exposes no recovery affordance and the Kratos recovery flow is disabled, so no recovery/verification email link can dead-end on a UI that cannot complete it. The recovery action/schema/provider/Go-endpoint plumbing stays dormant for that follow-up | ✅ `infra/kratos/kratos.yml` sets `recovery.enabled: false`; ✅ `AuthPanel.tsx` renders only sign-in/sign-up (no forgot-password entry); ✅ `docker compose --env-file deploy/compose.env.example config` renders no `SELFSERVICE_FLOWS_RECOVERY_UI_URL`/`VERIFICATION_UI_URL`; ✅ `npm run typecheck` + `npm run lint` clean; the dormant recovery endpoint is still covered by `go test ./...` (recovery status mapping / handler 502) |
...
| 9p | 2 | Direct Kratos self-service auth paths cannot bypass the Go API. The public Kratos self-service/session surface is not exposed at the nginx edge, and Kratos public (`4433`) is not bound to the host in production, so `/self-service/{registration,login}` and `/sessions/*` cannot be reached to sidestep the Go handler's duplicate-identifier sanitization + auth rate limit; recovery/verification browser flows are deferred, so no public self-service path is needed yet | ✅ both `infra/nginx-host/capsulezero.app.conf` and `infra/nginx/conf.d/capsulezero.conf` return `404` for `^~ /self-service/` and `^~ /sessions/`; ✅ `nginx:1.27-alpine nginx -t` passes for both configs (host vhost wrapped in the edge http context, rollback via `infra/nginx/nginx.conf`) with a temporary test cert; ✅ `docker compose --env-file deploy/compose.env.example config` shows the `kratos` service with **no** host `ports:` bind (public `4433` internal-only), while the dev override binds `127.0.0.1:4433` |
| 9q | 2 | Auth write throttling also covers requests forwarded by the Next.js server actions over the private network (which never traverse the edge `limit_req`), and keys on a **spoof-resistant, Cloudflare-correct** client address: the nginx `realip` module (Cloudflare ranges + `real_ip_header CF-Connecting-IP`) makes `$remote_addr`/`$binary_remote_addr` the true client even behind a Cloudflare POP — so the edge `limit_req` no longer shares one bucket across a POP, and a direct-to-origin `CF-Connecting-IP` forgery is ignored. The Go API rate-limits `/api/auth/{registration,login,recovery}` per client (10/min, burst 10) on the trusted `X-Capsule-Client-IP` header only; the web boundary derives that solely from `X-Real-IP` (nginx-set from the realip-corrected `$remote_addr`, overwritten every request — never `CF-Connecting-IP` or raw `X-Forwarded-For`), and nginx overwrites any client-supplied `X-Capsule-Client-IP` on `/api/*` | ✅ `docker run --rm -v "$PWD/api:/src" -w /src golang:1.25.11-bookworm go test ./...` passes `internal/ratelimit` (burst-then-429, time-based refill, per-key isolation, idle-bucket eviction, 429 envelope + `Retry-After`, distinct trusted clients, **raw `X-Forwarded-For` ignored**, trusted-header-preferred, RemoteAddr fallback); ✅ `go vet ./...` clean; ✅ `nginx -t` passes both configs with the `realip` block (Cloudflare ranges, `real_ip_header CF-Connecting-IP`, `real_ip_recursive on`) and `X-Capsule-Client-IP $remote_addr` on `/api/*`; ✅ `npm run typecheck` + `npm run lint` cover `apiFetch` deriving the IP from `x-real-ip` only |
| 10 | 2 | nginx routes public `/api/*` traffic to the Go API in both supported edge paths, throttles unauthenticated auth write endpoints before proxying, and does **not** expose the Kratos public self-service/session surface (deferred recovery/verification — see 9p): Docker nginx (`api:8080`) and active host/systemd nginx (`127.0.0.1:8080`) | ✅ `docker compose --env-file deploy/compose.env.example config`; ✅ `nginx:1.27 nginx -t` against `infra/nginx-host/capsulezero.app.conf` (host) and `infra/nginx/conf.d/capsulezero.conf` via `infra/nginx/nginx.conf` (rollback) with a temporary test cert; ✅ config grep verifies `limit_req_zone` plus throttled `/api/auth/(registration|login|recovery)` locations and `return 404` for `/self-service/` + `/sessions/` in both Docker and host nginx |
