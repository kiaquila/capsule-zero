...
6. Add plain `postgres:16` to compose with persistent volume and healthcheck. API connections use direct Postgres URLs in v0.1; PgBouncer and pgvector are deferred by ADR-007.
7. Add `kratos` to compose with Resend SMTP courier in prod and MailHog in `docker-compose.dev.yml` (reintroduced this phase).
8. Wire `infra/postgres/` init scripts to create the Kratos database and create app + Kratos roles; pgvector remains deferred by ADR-007.
...
14. Ship `api/migrations/0001_initial_schema.sql` (full schema + methodology seed). The API applies versioned SQL migrations on boot through its migration runner, idempotent.
...
22. Remove the retired Supabase provider modules from `/app` once Go API contexts own the matching domains.
23. Drop legacy Supabase env keys from `deploy/compose.env.example`, app env examples, and the droplet `.env`.
24. Delete `docker-compose.legacy-supabase.yml` if it still exists.
25. Keep `docker-compose.yml` building the `web` service from `/app`.
...
| --- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| 1   | 1     | `docker-compose.yml` declares `nginx` and `web` as separate `services:` blocks with explicit networks, volumes, and healthchecks                                                                                                                                                                                                                                            | `yq '.services' docker-compose.yml` shows `nginx` and `web`                                              |
...
| 4   | 1     | On the droplet: host nginx owns the edge — `systemctl is-active nginx` returns `active` and `systemctl is-active caddy` returns `inactive`; `docker compose ps` lists `web` as `(healthy)` within 60 s of `docker compose up -d` (the rollback compose edge is exercised separately — `sudo systemctl stop nginx`, then `docker compose --profile docker-edge up -d nginx`) | shell output committed to PR description after first droplet rollout                                     |
...
| 7   | 1     | Negative scenario 2 (`web` exits) returns 502 for `https://capsulezero.app/en`; `docker compose ps` shows `web` unhealthy                                                                                                                                                                                                                                                   | shell output collected during droplet smoke                                                              |
...
| 9   | 2     | `docker compose up -d postgres kratos` reaches healthy within 90 s; Postgres is the plain `postgres:16` image; `POSTGRES_URL` points directly at `postgres`                                                                                                                                                                                                                 | `docker compose ps` + `psql -c 'select version();'` output + env diff                                    |
...
| 20  | 6     | Retired Supabase provider/env surface removed; `/app` still builds the web service; no `/web` frontend target exists                                                                                                                                                                                                                                                        | `git ls-tree -r HEAD -- app web docker-compose.legacy-supabase.yml` + focused Supabase env/import search |
...
- **Retired Supabase provider still exists in some `/app` domains.** Until each bounded context moves to the Go API provider, some code paths may still import Supabase modules. Mitigation: keep the provider frozen, keep current provider modes to `mock`/`supabase` until the `api` provider lands, and remove Supabase domain by domain in Phase 6/follow-up slices.
