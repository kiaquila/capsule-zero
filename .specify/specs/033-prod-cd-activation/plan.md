...
| 12 | Rollback path works without crossing the Kratos schema boundary | `scripts/check-kratos-rollback-boundary.sh HEAD HEAD` accepts a same-runtime target; the negative `HEAD origin/main` probe on PR #101 rejects v26.2.0 → v1.3.1 before deployment. A live same-runtime `workflow_dispatch` remains exercised when first needed |
