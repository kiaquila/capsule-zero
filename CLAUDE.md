...
**Product work follows [PRODUCT-PLAN.md](PRODUCT-PLAN.md) until MVP is done (since 2026-07-16).** The product is being simplified around the outfit-combination algorithm (OPR), with the first value delivered *before* registration. The plan is canonical for product decisions until MVP — where it disagrees with any other doc on a product decision, the plan wins and you fix the drift in the same change. Three rules bind immediately: **coins are cancelled** as the monetization hypothesis (model reworked in plan Stage 4 — do not build features, specs, schemas, API/codegen, env, provisioning, or UI around coins; Stage 0 removed the active OpenAPI/generated/env/provisioning surface and retained provider/runtime references are superseded legacy only); **first value ships before signup** (guest onboarding → save-your-progress gate); **the algorithm must account for garment cut** (`basicity`, defined in `capsule-methodology.md` §6 but never implemented) — plan Stage 2. Founder decisions Q1/Q2/Q3/Q6 that gated the start of Stage 1 are closed (2026-07-21). **D5/D6 added 2026-07-24:** the preset catalog is built from **our own** photorealistic imagery — merchant/affiliate product photos may not be its basis (of four agreements checked only Awin grants a display licence, and only "without modification"; this is a risk-avoidance posture, not legal advice), and an item is three layers with the image as the **disposable** one. Q7 (catalog art direction) was **closed 2026-07-24** on a generation test — invisible mannequin for body garments, flat for trousers, three-quarter pair for shoes, flat-lay for bags/accessories, all alpha cutouts on one background and light; catalog content work is unblocked. Q8 (shared pool of user-imported merchant images) was **closed 2026-07-24 as option (б)** — the pool stays, built on the Polyvore pattern: the user performs the import, the platform makes **no additional derivative copies** of a third-party image, every image links back to its source, and notice-and-takedown plus a reasonably-implemented repeat-infringer policy are live. Two preconditions gate implementation: a compliance-scheme spec before any code, and an external legal review before launch — until both land, design and specify but do not implement that surface. **Q5** (basicity, gates Stage 2) is the only open question. The engineering contract (spec-kit, SENAR, TDD, PR gates), the stack, and AGENTS §8 are unchanged.

**Phase 5 — Development Sprint, in progress.** Runtime Phases 1–2 have landed (nginx + web; Postgres + Kratos + Go API + the `api` provider, PR #57); the current application slice is `.specify/specs/040-object-storage-upload-foundation/`. Asset buckets exist in project `15203114` / HEL and the Object-Locked backup bucket in project `15296835` / FSN. Runtime and backup-writer credentials live in bucketless key-only projects `15302873` and `15302925`, with cross-project policies. The runtime audit passed; the backup hybrid-policy audit proved normal puts plus read/delete/control/list/ACL denies and dangerous ACL header guards, while Hetzner/RGW still accepts Object Lock mode/retain-until/legal-hold headers on `PutObject`. Spec 047 accepts that bounded write-time storage-DoS/cost residual behind a root-owned fixed-header uploader: encrypted daily backups, Object Lock retention, and a full restore drill passed before activation; the server still cannot read or delete stored backups and the private `age` key remains off-server. Policy/CORS readback, absent backup CORS, protected env rotation, superseded-key revocation, and the post-revocation signed 10 MiB PUT/HEAD/GET/checksum/delete smoke passed. Personal-photo upload routes remain production-disabled by default until quota/cleanup and wardrobe attachment land. Cloudflare now fronts the apex and `www`, origin web traffic is Cloudflare-only, and operator/CI SSH is Tailscale-only with GitHub OIDC for the ephemeral CI node (spec 047). Every merge to `main` auto-deploys to `https://capsulezero.app` through `.github/workflows/cd-prod.yml` (spec 033) — there is no separate dev environment. Postgres is plain `postgres:16` (pgvector deferred, ADR-007). Full status and decisions: AGENTS.md → "Current Phase & Status" and "Phase 4 — Technical Architecture".
