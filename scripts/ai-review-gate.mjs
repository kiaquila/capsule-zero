...
const listPaginated = async (path) => {
  ...
  while (nextPath) {
    ...
    nextPath = nextMatch ? new URL(nextMatch[1]).pathname + new URL(nextMatch[1]).search : "";
  }
...
if (!prNumber) {
  throw new Error("AI Review gate requires a pull request context or AI_REVIEW_PR_NUMBER");
}
...
const findExistingTriggerComment = async () => {
  ...
  return comments.find((comment) => (comment.body || "").includes(metadataMarker)) || null;
};
...
const ensureTriggerComment = async () => {
  ...
  return request(`/repos/${owner}/${repo}/issues/${prNumber}/comments`, {
    method: "POST",
    body: JSON.stringify({
      body: buildTriggerComment(),
    }),
  });
};

const matchesMarker = (review) => {
  const body = review.body || "";

  return (
    review.commit_id === headSha &&
    body.includes(markerAgentLine) &&
...
const extractGeminiSeverity = (body) => {
  const imageAltMatch = body.match(/!\[(critical|high|medium|low)\]/i);
  if (imageAltMatch) {
    return imageAltMatch[1].toLowerCase();
  }

  const urlMatch = body.match(/\/(critical|high|medium|low)-priority\.svg/i);
  if (urlMatch) {
    return urlMatch[1].toLowerCase();
  }

  const textMatch = body.match(/\b(critical|high|medium|low)\b/i);
  if (textMatch) {
...
const classifyGeminiReview = async (review) => {
  ...
  const parsedSeverities = commentsForReview
  ...
  if (parsedSeverities.some((severity) => ["critical", "high", "medium"].includes(severity))) {
    const highestSeverity = ["critical", "high", "medium"].find((severity) =>
      parsedSeverities.includes(severity),
    );
...
const mapReviewState = (review) => {
  switch (review.state) {
    case "APPROVED":
      return {
        outcome: "pass",
        reason: "Selected reviewer approved the PR with no blocking findings.",
      };
    case "COMMENTED":
      return {
        outcome: "pass",
        reason: "Selected reviewer completed with advisory-only findings.",
      };
    case "CHANGES_REQUESTED":
      return {
        outcome: "fail",
        reason: "Selected reviewer reported blocking findings.",
      };
    default:
      return {
        outcome: "pending",
        reason: `Selected reviewer produced unsupported review state "${review.state}".`,
      };
  }
};
...
while (Date.now() < deadline) {
  if (selectedAgent === "claude") {
    ...
    const candidateReviews = triggerMode === "skip" ? reviews : recentReviews;
    const recentMatchingReview = pickLatestGeminiReview(candidateReviews);
