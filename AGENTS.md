...
**Tech stack:** Next.js 14+ App Router web frontend (`/app`), React Native mobile app (iOS + Android), Go modular monolith backend, nginx reverse proxy / API gateway, Ory Kratos auth, PostgreSQL 16 (plain `postgres:16` in v0.1 — pgvector deferred to the semantic-search slice, see ADR-007), Redis, Hetzner Object Storage, and an active Cloudflare front-door/CDN, all wired through docker-compose on a Hetzner Cloud server (migrated from DigitalOcean on 2026-07-02; Cloudflare activated on 2026-07-22).
...
| **4. Technical Architecture** | **PIVOTED TO PRODUCTION STACK** — Go modular monolith + nginx + Ory Kratos + Postgres + Redis + Hetzner Object Storage + active Cloudflare front-door + Resend; React Native replaces Flutter                     |
| 5. Development Sprint         | IN PROGRESS — **product work now follows [`PRODUCT-PLAN.md`](PRODUCT-PLAN.md) until MVP is done**; runtime Phases 1–2 landed (nginx + web; Postgres + Kratos + Go API + `api` provider, PR #57); storage slice `.specify/specs/040-object-storage-upload-foundation/`; hardened Cloudflare/Tailscale/backup production edge `.specify/specs/047-production-edge-hardening/`; every merge to `main` deploys to `https://capsulezero.app` via prod CD (spec 033) |
| 6. QA & Soft Launch           | Upcoming                                                                                                                                                                           |
| 7. Commercial Launch          | Upcoming                                                                                                                                                                           |

**Product rebuild decision, 2026-07-16 ([`PRODUCT-PLAN.md`](PRODUCT-PLAN.md)):** product work is simplified around the outfit-combination algorithm (OPR) and re-sequenced into four stages: (1) selling landing + free pre-signup loop, (2) algorithm incl. garment cut/basicity, (3) simplification of the post-registration product, (4) unit economics and monetization. **Until MVP is done, `PRODUCT-PLAN.md` is canonical for product decisions** — where it and any other doc disagree on a product decision, the plan wins and the drift is fixed in the same change. Three consequences bind immediately: **first value is delivered before registration** (Duolingo pattern — guest onboarding, then a save-your-progress gate); **coins are cancelled as the monetization hypothesis** — do not design features, specs, schemas, API, codegen, env, provisioning, or UI around coins until Stage 4 decides the model; the active coin/Lava OpenAPI, generated-client, env, and provisioning surface was removed in Stage 0, while retained provider/runtime references are explicitly superseded legacy for Stage-4 deletion or replacement; **the algorithm must account for garment cut, not only colour** — the `basicity` score already defined in `capsule-methodology.md` §6 is currently unimplemented and must become a real algorithm input in Stage 2. The plan does not change the engineering contract (spec-kit, SENAR, TDD, PR gates), the stack, or AGENTS §8. Founder decisions Q1/Q2/Q3/Q6 that gated the start of Stage 1 are closed (2026-07-21). **Q5** (basicity, gates Stage 2) is the only open question. **Q7** (catalog art direction) closed 2026-07-24: invisible mannequin for body garments, flat presentation for trousers, three-quarter pair for shoes, flat-lay for bags and accessories, all as alpha cutouts on one shared background and light. **Q8** (shared user-import pool) closed 2026-07-24 as **option (б)** — the pool stays and is built on the Polyvore pattern; its two preconditions survive the closure and gate implementation: a compliance-scheme spec and an external legal review.
...
**Storage provider decision, 2026-07-10 (spec 039):** DigitalOcean Spaces is superseded by **Hetzner Object Storage**. v0.1 storage uses S3-compatible Hetzner buckets, not the server root disk, a one-node MinIO, or a Cloud Volume. There is no built-in object-storage CDN in v0.1; public catalog assets use native object URLs until a separate catalog-CDN slice even though the application front door is now Cloudflare. Hetzner Object Storage has no default data-at-rest encryption, so encrypted database backups are mandatory and personal-photo storage needs the explicit ADR-003 security posture.

**Storage implementation decision, 2026-07-10 (spec 040; key hardening 2026-07-11; backup activation 2026-07-22 in spec 047):** direct presigned PUT/GET for private personal-photo originals is founder-accepted with the ADR-003 controls. Asset buckets are provisioned in HEL under Hetzner project `15203114`; the Object-Locked backup bucket is isolated in FSN under project `15296835`. Runtime and backup-writer credentials now live in bucketless key-only projects `15302873` and `15302925`. The runtime policy grants private-bucket listing and put/get/delete only for `item-originals/*` plus `smoke/spec-040/*`, while public catalog keeps anonymous reads and denies that runtime principal `s3:*`. The backup policy is a hybrid boundary: normal puts under `postgres/*` work; live probes deny object/version reads, ACL/retention/legal-hold get/put, deletes, governance bypass, bucket/version/multipart listing, and policy/CORS/lock-config reads, while header conditions reject dangerous canned ACLs and AllUsers grant-read. Hetzner/RGW nevertheless accepts a `PutObject` carrying Object Lock mode, retain-until, or legal-hold headers despite the related action denies. This does not permit reading or deleting existing data, but it leaves a bounded write-time storage-DoS/cost-amplification residual. The accepted activation boundary is a root-owned uploader that emits a fixed local header set with no caller-controlled metadata, encrypts every stream with `age`, and holds only the upload-only credential; the private key remains off-server. The daily timer, Object Lock retention, and a full restore drill passed before activation. Policy/CORS readback, the caveated live audits, protected env rotation, old runtime/backup keys plus both temporary policy-operator deletions, exact-origin/attacker-origin CORS probes, and the post-revocation signed 10 MiB PUT/HEAD/GET/checksum/delete smoke passed with `OBJECT_STORAGE_UPLOADS_ENABLED=false`. Personal-photo upload routes remain disabled until owner quota, orphan cleanup, and wardrobe attachment land. Credential and presigned-URL values must never enter repo/chat/evidence.

**Preset-catalog and item-model decision, 2026-07-24 ([`PRODUCT-PLAN.md`](PRODUCT-PLAN.md) D5/D6):** the preset catalog we produce — the default source of items for guests and new users — is built from **our own** photorealistic imagery. Merchant/affiliate product photos may **not** be its basis: of the four agreements checked (Awin, CJ, Rakuten, Amazon) only Awin grants a written display licence and only "without modification" — which background removal and a unified art direction would breach — while CJ forbids copying, Rakuten is silent, and Amazon bars retention beyond 24h; any Awin advertiser may revoke and demand immediate removal without cause. Networks we have not checked (Skimlinks, Sovrn, Tradedoubler, Impact) are not covered by that finding. This is a risk-avoidance posture from an agent's reading of public terms, **not** legal advice: declining to use others' images needs no lawyer, but any decision to rely on them does. Affiliate feeds are therefore a *candidate* for click monetization (Stage 4, D2) — never a source of catalog imagery. **D6:** an item is three layers — durable record, attributes (`category`, `colorId`, role/slot), and a **disposable** image, so the item survives any image being replaced or withdrawn. The catalog art direction is settled (plan §4-Q7, closed 2026-07-24 on a generation test — invisible mannequin for body garments, flat for trousers, three-quarter pair for shoes, flat-lay for bags/accessories, alpha cutouts on one background and light). The shared user-import pool (plan §4-Q8) is **retained** — the founder chose **option (б)** on 2026-07-24: US-011/US-012/US-025 and §6 of this file stand, built on the Polyvore pattern. Because we still hold no licence to republish merchant imagery, the pattern is the whole basis: the **user** performs the import, the platform generates **no additional derivative copies** of a third-party image (the conduct that defeated Polyvore's summary judgment), every image carries a link back to its source, and notice-and-takedown plus a reasonably-implemented repeat-infringer policy must be live. Two preconditions gate implementation and did not disappear with the decision: a **compliance-scheme spec** before any code, and an **external legal review** before launch. Until both land, do not implement that surface in any schema, migration, or code — design and specification are unblocked. Canonical affected-file list and inventory command: PRODUCT-PLAN §7 (Stage-3 row).

**Production edge hardening decision, 2026-07-22 (spec 047):** the former Stage-2 Cloudflare deferral is superseded. `capsulezero.app` and `www` are proxied through Cloudflare with Full (strict) TLS and DNSSEC; the origin web firewall accepts only Cloudflare ranges. Public TCP/22 is closed. Operators and GitHub-hosted deploy runners use Tailscale, with CI receiving a short-lived `tag:ci` identity from GitHub OIDC and permission only to reach the production host on TCP/22. Bot Fight Mode stays disabled because the Free-plan control challenged the API health monitor; scoped rate limiting and the default WAF/DDoS controls remain active.
...
.specify/
  ...
  specs/
    ...
    040-object-storage-upload-foundation/
      ...
      tasks.md           ← Process Memory
    047-production-edge-hardening/
      spec.md            ← Active Cloudflare/Tailscale/backup production-edge contract
      plan.md            ← Config, live-edge, restore, and merge-readiness verification
      tasks.md           ← Process Memory
```
...
> **Retained, with conditions (2026-07-24).** This principle stands. PRODUCT-PLAN D5/D6 make the preset catalog (our own imagery) the default item source and put guest photo upload out of Stage 1 — a sequencing change, not a removal. On the third method: we hold no licence to republish merchant product photos (of four agreements checked only Awin grants one, and only "without modification"), and the founder resolved **§4-Q8** as **option (б)** — keep it, on the Polyvore pattern. Implementation is gated on a compliance-scheme spec and an external legal review; until both land, do not implement the shared-merchant-image surface in schema, migration, or code.
...
- ~~Spaceship DNS pointed at Cloudflare; Cloudflare proxy enabled for `capsulezero.app`~~ — done 2026-07-22 (spec 047): apex + `www` proxied, Full (strict) TLS and DNSSEC active, origin web ports restricted to Cloudflare ranges, and real-IP restoration live.
...
- Hetzner Object Storage bucket provisioning, exact `https://capsulezero.app` asset CORS, absent backup CORS, bucketless key-only projects, cross-project policies, caveated live audits, protected env rotation, superseded-key revocation, and the post-revocation signed 10 MiB smoke are complete (assets: `15203114` / HEL; backups: `15296835` / FSN; runtime keys: `15302873`; backup keys: `15302925`). Uploads still wait for quota/cleanup/wardrobe attachment. Encrypted daily backup automation is active after fixed-header uploader hardening, explicit residual-risk acceptance, retention setup, and a successful restore drill (spec 047).
- Ship the remaining `.specify/specs/024-production-stack-runtime/` phases. Phases 1–2 deploy via prod CD (spec 033), spec 040 advances the Object Storage subset, and spec 047 closes the Cloudflare/Tailscale/backup edge work; Redis, imgproxy, and observability remain.
