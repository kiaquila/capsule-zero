...
type DraftErrors = Partial<
  Record<"name" | "categoryId" | "colorHexes", string>
>;
...
export function MyItemsShell({ snapshot }: MyItemsShellProps) {
  ...
  const wardrobeItems = useMemo(
    () => items.filter(isWardrobeStatisticItem),
    [items],
  );
  const counts = useMemo(
    () => buildCounts(items, snapshot.navigation),
    [items, snapshot.navigation],
  );
  const categories = useMemo(
    () => buildCategoryFilters(wardrobeItems),
    [wardrobeItems],
  );
  const colors = useMemo(
    () => buildColorFilters(wardrobeItems),
    [wardrobeItems],
  );
  const knownColors = useMemo(
    () =>
      uniqueColorPoints([
        ...snapshot.colors,
        ...wardrobeItems.flatMap((item) => item.colorPoints),
      ]),
  ...
  const moveSelectedItem = (
    status: Extract<ItemStatus, "for_sale" | "for_repair">,
  ) => {
    if (!draft?.id) {
      return;
    }
          ...
          <header className="dashboard-topbar my-items-topbar">
            <div>
              ...
              <p className="my-items-subtitle">
                {t("subtitle", { count: counts.myItems })}
              </p>
            </div>
